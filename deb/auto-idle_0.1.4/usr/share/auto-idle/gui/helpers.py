...
def enable_autostart():
    ...
    with open(AUTOSTART_FILE, "w") as f:
        f.write(
            f"[Desktop Entry]\n"
            f"Type=Application\n"
            f"Name=Auto Idle Power Switcher\n"
            f"Comment=Automatically switch power profiles based on idle time\n"
            f"Exec={APP_EXEC}\n"
            f"Icon={icon_path_for_mode('balanced')}\n"
            f"Terminal=false\n"
...
def apply_temperature_keyboard_rgb():
    ...
    if color == last_temp_color:
        return
    brightness = settings.temperature_rgb.get("brightness", "med")
    ...
    try:
        ...
        print(f"Temperature keyboard RGB applied: {color.upper()}, brightness={brightness}")
